...
import styles from './Chart10.css';
...
export class Chart10 extends Component {
  constructor(props) {
    super(props);
...
//build chart
    ...
    for(let k = 0; k < date.length + 1; k++) {
      ...
      for (let j = 0; j < projectName.length; j ++) {
        ...
        for(let i = 0; i < this.realData.length; i++) {
          ...
          if(!this.realData[i].hasOwnProperty('broker') || this.realData[i].broker == null) {
            continue;
          }
  ...
  render() {
    ...
    const config = {
      ...
      title: {
        text: '<h1 style="font-size: 30px">Broker Closed Won Sales</h1><br><h1 style="color: #828282">BY PROJECT  REFRESHED: 10/17/2016. 10:38:32 AM</h1><br><h2 style="color: #828282; margin-left: 50px;">Amount by Opportunity Referral Broker Account: Account Name and Project</h2>',
        align: 'left',
        y: 40
      },
      xAxis: {
